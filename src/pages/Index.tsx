...
const Index = () => {
  ...
  const refetchSourceMedia = useCallback(async () => {
    ...
    try {
      const promises = currentSourceSubredditsNames.map(name => 
        fetchRedditData(name, 'hot', null, 50)
          .then(d => extractMediaUrls(d.posts))
          .catch(() => [])
      );
  ...
  const appendPostsToFeed = useCallback((targetPosts: any[]) => {
    ...
    targetPosts.forEach(post => {
      ...
      const isTrulyMediaPost = pData.post_hint === 'image' || 
                             pData.post_hint === 'hosted:video' || 
                             pData.post_hint === 'rich:video' || 
                             pData.is_video ||
                             pData.is_gallery ||
                             (pData.preview && pData.preview.images && pData.preview.images.length > 0 && 
                              (pData.domain !== 'self.' + pData.subreddit.toLowerCase() && !pData.url.includes('/comments/')));

      if (isTrulyMediaPost) {
        if (allSourceMediaUrls.length < 10 && currentSourceSubredditsNames.length > 0 && 
            !isRefetchingSources && !isLoadingInitialSources) {
          refetchSourceMedia();
        }
  ...
  const fetchInitialData = useCallback(async () => {
    ...
    const sourceSubs = sourceSubsRaw
      .split(',')
    ...
    try {
      // Fetch source media
      const srcPromises = sourceSubs.map(name => 
        fetchRedditData(name, 'hot', null, 75)
          .then(d => extractMediaUrls(d.posts))
          .catch(err => {
    ...
    targetSubreddit, sourceSubreddits, sortMode, topTimeFilter, 
    appendPostsToFeed, clearMessage, displayMessage
  ]);
  ...
  const loadMoreTargetPosts = useCallback(async () => {
    ...
    isLoadingMore, noMoreTargetPosts, currentTargetSubredditName, targetAfter,
    sortMode, topTimeFilter, appendPostsToFeed, displayMessage, clearMessage
  ]);
  ...
  return (
    <div className="app-bg app-text">
      <div className="main-container container mx-auto min-h-screen flex flex-col">
        ...
        <section className="controls-section control-panel-bg shadow-md sticky top-2 z-50">
          <div className="controls-grid grid md:grid-cols-3 items-stretch">
            ...
            <div className="flex items-end">
              <button 
                className={`primary-button w-full focus:outline-none ${isLoadingPosts ? 'opacity-50 cursor-not-allowed' : ''}`}
          ...
          <div className="controls-sub-grid border-t grid grid-cols-1 md:grid-cols-3 items-stretch">
            <div>
              ...
              <div className="flex space-x-1.5">
                ...
                <button 
                  className={`secondary-button ${viewMode === 'gallery' ? 'active' : ''}`}
            ...
            {sortMode === 'top' && (
              <div>
                ...
                <select 
                  ...
                  className="form-input select-filter-arrow w-full"
                  value={topTimeFilter}
                  onChange={(e) => setTopTimeFilter(e.target.value as TopTimeFilter)}
                >
        ...
        <footer className="main-footer text-center border-t">
          <p>Gaslighter App &copy; 2025. Enjoy the altered reality responsibly.</p>
        </footer>
      </div>
